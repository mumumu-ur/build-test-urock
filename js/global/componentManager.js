  ...
  initComponent: function (componentName, element) {
    ...
    if (component && typeof component.init === "function") {
      try {
        ...
        console.error(
          `[ComponentManager] 컴포넌트 초기화 오류 (${componentName}):`,
          error
        );
      }
    } else {
      console.warn(
        `[ComponentManager] 등록되지 않은 컴포넌트: ${componentName}`
      );
    }
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("componentLoaded", function (event) {
    ...
    if (!component) {
      console.warn(
        "[ComponentManager] 컴포넌트 이름이 이벤트에 포함되지 않았습니다."
      );
      return;
    }
      ...
      initAttempts.forEach((delay, index) => {
        setTimeout(() => {
          if (typeof window.reInitMobileMenu === "function") {
            ...
            console.warn(
              "[ComponentManager] reInitMobileMenu 함수를 찾을 수 없음"
            );
          }
        }, delay);
      });
