...
function initInquiryForm() {
  ...
  if (typeof $ === "undefined") {
    console.warn(
      "jQuery가 로드되지 않았습니다. 일부 기능이 제한될 수 있습니다."
    );
    return;
  }
  ...
  $("select[name='inquiry-type']")
    ...
    .change(function () {
      ...
      if (prev_first != selectedValue) {
        ...
        if (
          prev_first == "기술지원" ||
          (selectedValue == "기술지원" && prev_first != "선택하세요")
        ) {
          isChanged = confirm(
            "문의유형을 변경하면 초기화될 수 있습니다.\n변경하시겠습니까?"
          );
          if (isChanged) {
      ...
      if (selectedValue == "솔루션 문의") {
        ...
        secondSelect.append(
          '<option value="휴대폰 안심 지우개">휴대폰 안심 지우개</option>'
        );
        secondSelect.append(
          '<option value="디지털포렌식">디지털포렌식</option>'
        );
        secondSelect.append(
          '<option value="보안 통제 시스템">보안 통제 시스템</option>'
        );
        secondSelect.append(
          '<option value="업무 지원 시스템">업무 지원 시스템</option>'
        );
      }
      ...
      if (selectedValue == "서비스 문의") {
        ...
        $(".selt003").show().prop("disabled", true);
        secondSelect.append(
          '<option value="포렌식 컨설팅">포렌식 컨설팅</option>'
        );
        secondSelect.append('<option value="포렌식 교육">포렌식 교육</option>');
        secondSelect.append(
          '<option value="데이터 삭제 센터">데이터 삭제 센터</option>'
        );
      }
  ...
  $("select[name='second']").change(function () {
    ...
    if (selectedValue == "포렌식 컨설팅") {
      ...
      thirdSelect.append(
        '<option value="포렌식 분석 서비스">포렌식 분석 서비스</option>'
      );
      thirdSelect.append(
        '<option value="디지털포렌식 랩 구축">디지털포렌식 랩 구축</option>'
      );
      thirdSelect.append(
        '<option value="국제 표준화 인증">국제 표준화 인증</option>'
      );
    }
...
function initializePageFeatures(pageInfo) {
  ...
  try {
    ...
    console.error(
      `[Solution] ${type}-${page} 특별 기능 초기화 중 오류:`,
      error
    );
  }
}
